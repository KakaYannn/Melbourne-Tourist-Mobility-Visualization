...
pois <- st_read('./data/landmarks-and-places-of-interest-including-schools-theatres-health-services-spor.geojson')
...
# read data
stops <- st_read('./data/public_transport_stops.geojson')
stations <- read.csv('./data/annual_metropolitan_train_station_entries_fy_2024_2025.csv')
lines <- st_read('./data/public_transport_lines.geojson')
...
# 1) File paths
ped_counts_path <- "./data/pedestrian-counting-system-monthly-counts-per-hour.xlsx"
sensor_csv_local <- "./data/pedestrian-counting-system-sensor-locations.csv"
...
load_data <- function() {
  file_path <- "./data/Data_Tables_LGA_Recorded_Offences_Year_Ending_June_2025.xlsx"
...
load_suburb_boundaries <- function() {
  ...
  if (file.exists('./data/vic_suburbs.geojson')) {
    suburbs_all <- st_read('./data/vic_suburbs.geojson', quiet = TRUE) %>%
      st_transform(4326)
    return(suburbs_all)
  }
  
  # Option 4: Try backup file if main file doesn't exist
  if (file.exists('./data/vic_suburbs.geojson.backup')) {
    suburbs_all <- st_read('./data/vic_suburbs.geojson.backup', quiet = TRUE) %>%
      st_transform(4326)
    return(suburbs_all)
  }
